...
```yaml
behavior_rules:
  give_up_after: 2_retries        # 何回失敗で諦めるか
...
の `behavior_rules.oneOf[1]` 参照。`panic_on` の enum や
`on_ambiguous_button` の enum は意図的に絞ってある（追加したい場合は
`custom` に書くか、スキーマ側で追加する）。
